    ...
    <>
      ...
      <div className={`border-t border-gray-800 ${collapsed ? 'px-2 py-4' : 'px-4 py-4'}`}>
        {!collapsed && user?.role === 'ADMIN' && (
          <Link href="/admin" className="block text-xs text-yellow-400 hover:text-yellow-300 mb-3 transition-colors py-2" onClick={onClose}>
            → Admin Panel
          </Link>
        )}
        {!collapsed && (
      ...
      >
        <div className="flex-1 flex flex-col overflow-y-auto">
          <div className={`flex items-center border-b border-gray-800 ${collapsed ? 'px-2 py-4 justify-center flex-col gap-1' : 'justify-between px-4 py-4 md:py-5'}`}>
            {collapsed ? (
              <>
                <Link href="/sales" className="flex justify-center" onClick={() => setMobileOpen(false)}>
              ...
              <Link href="/sales" className="flex items-center gap-2" onClick={() => setMobileOpen(false)}>
