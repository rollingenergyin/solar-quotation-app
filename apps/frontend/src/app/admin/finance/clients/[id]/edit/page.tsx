...
export default function EditClientPage() {
  ...
  useEffect(() => {
    ...
    api<{ name: string; gstin: string | null; contact: string | null; address: string | null }>(
      ...
      .then((c) => setForm({ name: c.name, gstin: c.gstin ?? '', contact: c.contact ?? '', address: c.address ?? '' }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api(`/finance/clients/${id}`, { method: 'PUT', body: JSON.stringify(form) });
      router.push('/admin/finance/clients');
    } catch (e) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-xl mx-auto">
      <Link href="/admin/finance/clients" className="text-sm text-gray-500 hover:text-gray-700 mb-4 inline-block">← Clients</Link>
