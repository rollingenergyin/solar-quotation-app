              ...
              <tbody>
                {leads.map((lead) => (
                  <tr key={lead.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/admin/crm/${lead.id}`} className="text-xs text-blue-600 hover:text-blue-800 font-medium whitespace-nowrap">View →</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={sources} dataKey="count" nameKey="source" cx="50%" cy="50%" outerRadius={75}
                  label={({ source, percent }) => `${source} ${Math.round((percent ?? 0) * 100)}%`}>
                  ...
                  <td className="py-2.5 pl-3 w-28">
                    <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full rounded-full"
                        style={{ width: `${pct}%`, backgroundColor: STAGE_CHART_COLORS[i % STAGE_CHART_COLORS.length] }} />
                    </div>
                  </td>
                </tr>
              );
...
function CampaignsTab() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowForm(false);
      setForm({ name: '', type: 'BROADCAST', channel: 'whatsapp', templateId: '', scheduledAt: '', targetFilter: '{"stage":"NEW"}' });
    },
  });
  ...
  return (
    <div>
      ...
      {isLoading ? <div className="text-center text-gray-400 py-10">Loading…</div> : (
        <div className="space-y-3">
          {campaigns.length === 0 && (
            <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center text-gray-400">No campaigns yet.</div>
          )}
          {campaigns.map((c) => (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-start justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 my-8">
            ...
            <div className="space-y-3">
              ...
              <input placeholder="Template ID" value={form.templateId}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-start justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-6 my-8">
            ...
            <div className="space-y-3">
              ...
              {[['contentEn','English *'], ['contentHi','Hindi'], ['contentMr','Marathi']].map(([key, label]) => (
                <div key={key}>
                  ...
                  <textarea value={form[key as keyof typeof form]}
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={() => setShowForm(false)} className="flex-1 text-sm border border-gray-200 rounded-lg py-2 text-gray-600">Cancel</button>
              <button
                onClick={() => createMutation.mutate({ ...form, variables: form.variables.split(',').map((v) => v.trim()).filter(Boolean) })}
    ...
    <div className="max-w-2xl">
      ...
      <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 p-10 text-center mb-5">
        ...
        <p className="text-xs text-gray-400 mb-4">
          Required columns: <code className="bg-gray-100 px-1.5 py-0.5 rounded">name</code>,{' '}
          <code className="bg-gray-100 px-1.5 py-0.5 rounded">phone</code> — optional: email, city, state, kw, language
        </p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <span className={`ml-2 text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_COLOR_IMPORT[importStatus.status]}`}>
                {importStatus.status}
              </span>
            </div>
